...
status: "active"
owners: []
triggers: ["tests","fuzz","Clippy","regression"]
prerequisites: ["task-planning"]
related_docs: ["TESTING.md","tests/README.md",".github/workflows/boot-smoke.yml"]
...
description: "Use when working on tests, fuzz, Clippy; provides the FinnOS-specific test strategy workflow and evidence gates."
---
# Test Strategy

## 1. Name

`test-strategy` (Foundations; skill maturity: operational).

## 2. Purpose

Choose layered regression evidence and protect CI gates.

## 3. When to use this skill

Use for requests mentioning `tests`, `fuzz`, `Clippy`, `regression`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.
...
An evidence-backed test strategy result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Make the defect observable
2. Put pure policy in host tests and hardware behavior in QEMU
3. Isolate feature artifacts
4. Add malformed/exhaustion/teardown cases
5. Run narrow then aggregate gates and document gaps
...
Preserve the boundary described by the current state: 63 Rust and 33 Python tests plus eight x86 QEMU modes passed at the audit snapshot; counts are historical, not permanent assertions. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Run narrow then aggregate gates and document gaps` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Make the defect observable` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: 63 Rust and 33 Python tests plus eight x86 QEMU modes passed at the audit snapshot; counts are historical, not permanent assertions.
...
Request: "Work on tests." Correct response: begin by `make the defect observable`, then `put pure policy in host tests and hardware behavior in qemu`, and require evidence for `run narrow then aggregate gates and document gaps` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
