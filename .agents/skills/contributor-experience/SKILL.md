...
status: "active"
owners: []
triggers: ["contributor","onboarding","good first issue"]
prerequisites: ["documentation-maintenance","github-project-management","build-environment-management"]
related_docs: ["CONTRIBUTING.md","BUILDING.md","TESTING.md","SUPPORT.md"]
category: "Quality, release, and docs"
conditional_skills: []
implementation_gates: []
related_milestones: ["M0 Reproducible Build","M6 Developer Preview"]
...
description: "Use when working on contributor, onboarding, good first issue; provides the FinnOS-specific contributor experience workflow and evidence gates."
---
# Contributor Experience

## 1. Name

`contributor-experience` (Quality, release, and docs; skill maturity: operational).

## 2. Purpose

Keep setup, first build/boot, issues, debugging, review, support, and AI contribution expectations executable.

## 3. When to use this skill

Use for requests mentioning `contributor`, `onboarding`, `good first issue`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.
...
An evidence-backed contributor experience result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Test instructions from clean environment
2. Minimize first successful command path
3. Create bounded evidence-rich first issues
4. Link debugging/review/support policies
5. Require AI contributions to disclose tests, limitations, and handoff
...
Preserve the boundary described by the current state: Current onboarding supports x86 development but no SDK/application path; ownership slots are unassigned. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Require AI contributions to disclose tests, limitations, and handoff` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Test instructions from clean environment` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: Current onboarding supports x86 development but no SDK/application path; ownership slots are unassigned.
...
Request: "Work on contributor." Correct response: begin by `test instructions from clean environment`, then `minimize first successful command path`, and require evidence for `require ai contributions to disclose tests, limitations, and handoff` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
