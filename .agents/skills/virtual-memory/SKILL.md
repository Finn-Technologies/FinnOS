...
status: "active"
owners: []
triggers: ["paging","virtual memory","page fault","address space"]
prerequisites: ["physical-memory-management","cross-architecture-design","boot-kernel-hardening"]
related_docs: ["kernel/src/arch/x86_64/paging.rs","docs/architecture/x86_64-virtual-memory.md"]
...
description: "Use when working on paging, virtual memory, page fault; provides the FinnOS-specific virtual memory workflow and evidence gates."
---
# Virtual Memory

## 1. Name

`virtual-memory` (Boot and kernel; skill maturity: operational).

## 2. Purpose

Manage kernel and future process mappings, faults, permissions, ownership, and TLB behavior.

## 3. When to use this skill

Use for requests mentioning `paging`, `virtual memory`, `page fault`, `address space`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.
...
An evidence-backed virtual memory result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Define mapping owner and lifetime
2. Validate canonical/aligned ranges and overflow
3. Enforce W^X, NX, user/supervisor, and guards
4. Specify TLB invalidation and table reclamation
5. Test expected faults and cross-address-space isolation
...
Preserve the boundary described by the current state: x86 private four-level 4 KiB mappings enforce supervisor W^X/NX/null/guards; user mappings, address-space objects, COW, shared memory, and ARM tables are absent. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Test expected faults and cross-address-space isolation` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Define mapping owner and lifetime` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: x86 private four-level 4 KiB mappings enforce supervisor W^X/NX/null/guards; user mappings, address-space objects, COW, shared memory, and ARM tables are absent.
...
Request: "Work on paging." Correct response: begin by `define mapping owner and lifetime`, then `validate canonical/aligned ranges and overflow`, and require evidence for `test expected faults and cross-address-space isolation` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
