---
name: "userspace-isolation"
title: "Userspace Isolation"
version: 1
status: "active"
owners: []
triggers: ["userspace","ring 3","EL0","process isolation"]
prerequisites: ["virtual-memory","cross-architecture-design","boot-kernel-hardening","unsafe-rust-low-level-safety"]
related_docs: ["docs/architecture/processes.md","ROADMAP.md","SECURITY.md"]
category: "Boot and kernel"
conditional_skills: ["scheduler-thread-development"]
implementation_gates: ["Depends on M1 architecture parity and preemptible/blocking thread foundations."]
related_milestones: ["M2 Core Kernel","M3 Userspace Foundation"]
...
description: "Use when working on userspace, ring 3, EL0; provides the FinnOS-specific userspace isolation workflow and evidence gates."
---
# Userspace Isolation

## 1. Name

`userspace-isolation` (Boot and kernel; skill maturity: planning-gated).

## 2. Purpose

Create EL0/ring-3 processes, address spaces, stacks, executable state, fault containment, and resource limits.

## 3. When to use this skill

Use for requests mentioning `userspace`, `ring 3`, `EL0`, `process isolation`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.

## 4. When not to use this skill

Depends on M1 architecture parity and preemptible/blocking thread foundations. Do not load it only because a future FinnOS document mentions the subsystem.
...
- scheduler-thread-development and the affected platform skill during implementation

Implementation gates are roadmap/runtime conditions, not additional documents to load automatically:

- Depends on M1 architecture parity and preemptible/blocking thread foundations.
...
An evidence-backed userspace isolation result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Define process/thread ownership
2. Build user page tables and stack
3. Enter/return safely per architecture
4. Terminate only faulting process
5. Test kernel/peer isolation, invalid pointers, exhaustion, and cleanup
...
Preserve the boundary described by the current state: All current tasks are ring 0 in one x86 address space; ARM has no implementation. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Test kernel/peer isolation, invalid pointers, exhaustion, and cleanup` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Define process/thread ownership` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: All current tasks are ring 0 in one x86 address space; ARM has no implementation.
...
- Do not disregard the roadmap/status gate: Depends on M1 architecture parity and preemptible/blocking thread foundations.
...
Request: "Work on userspace." Correct response: begin by `define process/thread ownership`, then `build user page tables and stack`, and require evidence for `test kernel/peer isolation, invalid pointers, exhaustion, and cleanup` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
