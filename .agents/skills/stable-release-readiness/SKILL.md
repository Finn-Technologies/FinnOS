...
status: "active"
owners: []
triggers: ["stable release","1.0","release readiness"]
prerequisites: ["release-engineering","threat-modelling","evidence-status-reporting","status-roadmap-updates"]
related_docs: ["STATUS.md","RELEASING.md","SUPPORTED_PLATFORMS.md"]
category: "Quality, release, and docs"
conditional_skills: ["secure-updates-recovery","ui-ux-review"]
implementation_gates: ["Not actionable as a release approval until M0-M7 acceptance evidence exists."]
related_milestones: ["M8 Stable 1.0"]
...
description: "Use when working on stable release, 1.0, release readiness; provides the FinnOS-specific stable release readiness workflow and evidence gates."
---
# Stable Release Readiness

## 1. Name

`stable-release-readiness` (Quality, release, and docs; skill maturity: planning-gated).

## 2. Purpose

Audit maturity Level 6 across build, architecture, product, security, recovery, docs, hardware, and operations.

## 3. When to use this skill

Use for requests mentioning `stable release`, `1.0`, `release readiness`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.

## 4. When not to use this skill

Not actionable as a release approval until M0-M7 acceptance evidence exists. Do not load it only because a future FinnOS document mentions the subsystem.
...
- ui-ux-review, secure-updates-recovery, hardware, storage, and network skills for their release gates

Implementation gates are roadmap/runtime conditions, not additional documents to load automatically:

- Not actionable as a release approval until M0-M7 acceptance evidence exists.
...
An evidence-backed stable release readiness result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Freeze supported scope
2. Audit each STATUS maturity criterion with evidence
3. Require x86/ARM or explicitly narrowed supported architecture policy
4. Qualify install/update/recovery/security/hardware
5. Reject release with open P0/P1 or unsupported claims
...
Preserve the boundary described by the current state: FinnOS is Level 0 for x86 and partial Level 1; every product/release gate remains open. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Reject release with open P0/P1 or unsupported claims` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Freeze supported scope` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: FinnOS is Level 0 for x86 and partial Level 1; every product/release gate remains open.
...
- Do not disregard the roadmap/status gate: Not actionable as a release approval until M0-M7 acceptance evidence exists.
...
Request: "Work on stable release." Correct response: begin by `freeze supported scope`, then `audit each status maturity criterion with evidence`, and require evidence for `reject release with open p0/p1 or unsupported claims` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
