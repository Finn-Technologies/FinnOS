...
status: "active"
owners: []
triggers: ["interrupt","exception","trap frame","IRQ"]
prerequisites: ["cross-architecture-design","synchronization-concurrency","unsafe-rust-low-level-safety"]
related_docs: ["kernel/src/arch/x86_64/exceptions.rs","kernel/src/arch/x86_64/interrupts.rs","docs/architecture/x86_64-exceptions.md"]
...
description: "Use when working on interrupt, exception, trap frame; provides the FinnOS-specific interrupt and exception handling workflow and evidence gates."
---
# Interrupt and Exception Handling

## 1. Name

`interrupt-exception-handling` (Boot and kernel; skill maturity: operational).

## 2. Purpose

Design trap frames, controllers, IRQ lifecycle, fault policy, and preemption interaction safely.

## 3. When to use this skill

Use for requests mentioning `interrupt`, `exception`, `trap frame`, `IRQ`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.
...
An evidence-backed interrupt and exception handling result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Specify raw and normalized frame layouts
2. Define mask-route-ack-EOI-teardown ownership
3. Separate fatal kernel fault from containable user fault
4. Audit nesting/preemption/logging constraints
5. Test real faults, spurious IRQs, and forbidden contexts
...
Preserve the boundary described by the current state: x86 exception vectors and timer/spurious gates work; PIC is masked; no IOAPIC/device IRQs, user faults, nested scheduling, or ARM vectors. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Test real faults, spurious IRQs, and forbidden contexts` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Specify raw and normalized frame layouts` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: x86 exception vectors and timer/spurious gates work; PIC is masked; no IOAPIC/device IRQs, user faults, nested scheduling, or ARM vectors.
...
Request: "Work on interrupt." Correct response: begin by `specify raw and normalized frame layouts`, then `define mask-route-ack-eoi-teardown ownership`, and require evidence for `test real faults, spurious irqs, and forbidden contexts` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
