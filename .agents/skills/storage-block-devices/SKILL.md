---
name: "storage-block-devices"
title: "Storage and Block Devices"
version: 1
status: "active"
owners: []
triggers: ["block device","VirtIO block","partition","flush"]
prerequisites: ["task-planning","reliability-fault-injection","threat-modelling"]
related_docs: ["HARDWARE_SUPPORT.md","ROADMAP.md"]
category: "Drivers and hardware"
conditional_skills: ["driver-architecture","virtio"]
implementation_gates: ["Requires VirtIO/resource broker and userspace service model."]
...
description: "Use when working on block device, VirtIO block, partition; provides the FinnOS-specific storage and block devices workflow and evidence gates."
---
# Storage and Block Devices

## 1. Name

`storage-block-devices` (Drivers and hardware; skill maturity: planning-gated).

## 2. Purpose

Define request ownership, queues, alignment, cache/flush, errors, partitions, and first VirtIO block support.

## 3. When to use this skill

Use for requests mentioning `block device`, `VirtIO block`, `partition`, `flush`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.

## 4. When not to use this skill

Requires VirtIO/resource broker and userspace service model. Do not load it only because a future FinnOS document mentions the subsystem.
...
- virtio and driver-architecture for the first implementation

Implementation gates are roadmap/runtime conditions, not additional documents to load automatically:

- Requires VirtIO/resource broker and userspace service model.
...
An evidence-backed storage and block devices result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Define bounded asynchronous block requests
2. Specify buffer/DMA ownership and alignment
3. Implement flush/barrier/error semantics
4. Add partition parsing with hostile-input tests
5. Validate persistence/reboot and injected device failures
...
Preserve the boundary described by the current state: The kernel has no runtime block device; the FAT image is firmware-only boot media. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Validate persistence/reboot and injected device failures` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Define bounded asynchronous block requests` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: The kernel has no runtime block device; the FAT image is firmware-only boot media.
...
- Do not disregard the roadmap/status gate: Requires VirtIO/resource broker and userspace service model.
...
Request: "Work on block device." Correct response: begin by `define bounded asynchronous block requests`, then `specify buffer/dma ownership and alignment`, and require evidence for `validate persistence/reboot and injected device failures` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
