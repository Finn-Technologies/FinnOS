---
name: "uefi-bootloader-development"
title: "UEFI Bootloader Development"
version: 1
status: "active"
owners: []
triggers: ["UEFI","ELF loader","ExitBootServices","GOP"]
prerequisites: ["build-orchestration","boot-protocol-evolution","unsafe-rust-low-level-safety","qemu-boot-testing"]
related_docs: ["boot/uefi/","boot/README.md","docs/architecture/boot.md","docs/reference/boot-protocol.md"]
...
description: "Use when working on UEFI, ELF loader, ExitBootServices; provides the FinnOS-specific uefi bootloader development workflow and evidence gates."
---
# UEFI Bootloader Development

## 1. Name

`uefi-bootloader-development` (Boot and kernel; skill maturity: operational).

## 2. Purpose

Safely evolve firmware entry, ELF loading, GOP/memory handoff, and architecture-specific UEFI binaries.

## 3. When to use this skill

Use for requests mentioning `UEFI`, `ELF loader`, `ExitBootServices`, `GOP`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.
...
An evidence-backed uefi bootloader development result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Model malformed ELF/range cases first
2. Validate class/machine/segments/alignment/overflow/entry
3. Preserve allocations through ExitBootServices
4. Construct versioned BootInfo with explicit ownership
5. Boot-test exact loader/kernel pair
...
```bash
cargo test -p finn-boot-uefi --lib
...
Keep ELF machine/ABI and BOOTX64 versus future BOOTAA64 explicit.
...
Preserve the boundary described by the current state: Only x86 BOOTX64.EFI exists; entry membership is checked per executable segment; loader trusts firmware mappings and handoff storage. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Add table-driven malformed ELF tests and fuzz/property targets before expanding accepted inputs. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Model malformed ELF/range cases first` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: Only x86 BOOTX64.EFI exists; entry membership is checked per executable segment; loader trusts firmware mappings and handoff storage.
...
Request: "Work on UEFI." Correct response: begin by `model malformed elf/range cases first`, then `validate class/machine/segments/alignment/overflow/entry`, and require evidence for `boot-test exact loader/kernel pair` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
