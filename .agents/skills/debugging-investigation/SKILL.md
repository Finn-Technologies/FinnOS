...
title: "Debugging and Investigation"
version: 1
status: "active"
owners: []
triggers: ["panic","exception","hang","investigate"]
prerequisites: ["repository-orientation","test-strategy"]
related_docs: ["docs/development/debugging.md","kernel/docs/panic-policy.md","tools/finnlib/qemu.py"]
...
description: "Use when working on panic, exception, hang; provides the FinnOS-specific debugging and investigation workflow and evidence gates."
---
# Debugging and Investigation

## 1. Name

`debugging-investigation` (Foundations; skill maturity: operational).

## 2. Purpose

Localize failures across host, image, firmware, loader, protocol, kernel, and harness layers.

## 3. When to use this skill

Use for requests mentioning `panic`, `exception`, `hang`, `investigate`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.
...
An evidence-backed debugging and investigation result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Reproduce unchanged
2. Capture versions, command, status, and full serial
3. Identify first divergent marker
4. Symbolize exact ELF if possible
5. Minimize and bisect without discarding evidence
...
Never infer symbols from a different feature/profile ELF; do not use destructive Git operations. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Minimize and bisect without discarding evidence` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Reproduce unchanged` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: COM1 markers are primary runtime evidence; no maintained GDB workflow or persistent crash dump exists.
...
Request: "Work on panic." Correct response: begin by `reproduce unchanged`, then `capture versions, command, status, and full serial`, and require evidence for `minimize and bisect without discarding evidence` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
