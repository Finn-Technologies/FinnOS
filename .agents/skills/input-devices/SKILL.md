...
status: "active"
owners: []
triggers: ["keyboard","pointer","touch","VirtIO input","keymap"]
prerequisites: ["task-planning","threat-modelling","ui-ux-review"]
related_docs: ["HARDWARE_SUPPORT.md","UI_GUIDELINES.md"]
category: "Drivers and hardware"
conditional_skills: ["compositor-window-system","driver-architecture","ipc-capabilities","virtio"]
implementation_gates: ["Device event work requires driver/IPC foundations; UI routing requires compositor/session policy."]
related_milestones: ["M4 Devices and Storage","M5 Graphical Stack"]
...
description: "Use when working on keyboard, pointer, touch; provides the FinnOS-specific input devices workflow and evidence gates."
---
# Input Devices

## 1. Name

`input-devices` (Drivers and hardware; skill maturity: planning-gated).

## 2. Purpose

Create secure normalized keyboard, pointer, touch, keymap, text-input, and focus event paths.

## 3. When to use this skill

Use for requests mentioning `keyboard`, `pointer`, `touch`, `VirtIO input`, `keymap`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.

## 4. When not to use this skill

Device event work requires driver/IPC foundations; UI routing requires compositor/session policy. Do not load it only because a future FinnOS document mentions the subsystem.
...
- virtio, driver-architecture, and ipc-capabilities for device delivery
- compositor-window-system for focus routing

Implementation gates are roadmap/runtime conditions, not additional documents to load automatically:

- Device event work requires driver/IPC foundations; UI routing requires compositor/session policy.
...
An evidence-backed input devices result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Define raw device versus semantic input events
2. Implement VirtIO input first
3. Bound queues and validate ranges
4. Separate key events from text composition
5. Test focus/grab/revocation, multi-device, and malformed events
...
Preserve the boundary described by the current state: No input driver, event model, keymap, focus routing, or UI exists. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Test focus/grab/revocation, multi-device, and malformed events` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Define raw device versus semantic input events` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: No input driver, event model, keymap, focus routing, or UI exists.
...
- Do not disregard the roadmap/status gate: Device event work requires driver/IPC foundations; UI routing requires compositor/session policy.
...
Request: "Work on keyboard." Correct response: begin by `define raw device versus semantic input events`, then `implement virtio input first`, and require evidence for `test focus/grab/revocation, multi-device, and malformed events` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
