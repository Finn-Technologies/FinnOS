...
status: "active"
owners: []
triggers: ["desktop shell","launcher","notifications","lock screen"]
prerequisites: ["peony-toolkit-development","capability-privilege-review","ui-ux-review","threat-modelling"]
related_docs: ["UI_GUIDELINES.md","ROADMAP.md","SECURITY.md"]
category: "Graphics and Peony"
conditional_skills: ["audio-power-management","init-service-management"]
implementation_gates: ["Requires toolkit, session services, storage/settings, and power control."]
...
description: "Use when working on desktop shell, launcher, notifications; provides the FinnOS-specific desktop shell workflow and evidence gates."
---
# Desktop Shell

## 1. Name

`desktop-shell` (Graphics and Peony; skill maturity: planning-gated).

## 2. Purpose

Implement session/login, panel, launcher, notifications, window policy, settings, lock, shutdown, recovery, and accessibility.

## 3. When to use this skill

Use for requests mentioning `desktop shell`, `launcher`, `notifications`, `lock screen`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.

## 4. When not to use this skill

Requires toolkit, session services, storage/settings, and power control. Do not load it only because a future FinnOS document mentions the subsystem.
...
- init-service-management and audio-power-management during implementation

Implementation gates are roadmap/runtime conditions, not additional documents to load automatically:

- Requires toolkit, session services, storage/settings, and power control.
...
An evidence-backed desktop shell result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Define session and authentication boundary
2. Start minimal window policy/panel/launcher
3. Add privacy-safe notifications and settings
4. Integrate lock/shutdown/recovery capabilities
5. Test restart, keyboard-only use, locked data, and service failure
...
Preserve the boundary described by the current state: No login/session/shell/window manager/notification/settings/lock screen exists. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Test restart, keyboard-only use, locked data, and service failure` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Define session and authentication boundary` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: No login/session/shell/window manager/notification/settings/lock screen exists.
...
- Do not disregard the roadmap/status gate: Requires toolkit, session services, storage/settings, and power control.
...
Request: "Work on desktop shell." Correct response: begin by `define session and authentication boundary`, then `start minimal window policy/panel/launcher`, and require evidence for `test restart, keyboard-only use, locked data, and service failure` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
