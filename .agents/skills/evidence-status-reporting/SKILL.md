---
name: "evidence-status-reporting"
title: "Evidence and Status Reporting"
version: 1
status: "active"
owners: []
triggers: ["status","completion percentage","verified"]
prerequisites: ["repository-orientation"]
related_docs: ["STATUS.md","docs/audit/2026-07-16.md"]
...
description: "Use when working on status, completion percentage, verified; provides the FinnOS-specific evidence and status reporting workflow and evidence gates."
---
# Evidence and Status Reporting

## 1. Name

`evidence-status-reporting` (Foundations; skill maturity: operational).

## 2. Purpose

Classify implementation state with reproducible evidence and calibrated confidence.

## 3. When to use this skill

Use for requests mentioning `status`, `completion percentage`, `verified`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.
...
A claim table using verified complete, implemented-unverified, partial, prototype, stub, planned, broken, blocked, or unknown. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Name the claim and scope
2. Collect source, build, runtime, hardware, and CI evidence
3. Assign one classification
4. Record confidence/unknowns
5. Update canonical status only when justified
...
Preserve the boundary described by the current state: The audit distinguishes verified x86 paths from absent ARM64/product subsystems. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Update canonical status only when justified` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Name the claim and scope` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: The audit distinguishes verified x86 paths from absent ARM64/product subsystems.
...
Request: "Work on status." Correct response: begin by `name the claim and scope`, then `collect source, build, runtime, hardware, and ci evidence`, and require evidence for `update canonical status only when justified` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
