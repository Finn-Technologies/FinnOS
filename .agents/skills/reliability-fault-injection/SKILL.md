---
name: "reliability-fault-injection"
title: "Reliability and Fault Injection"
version: 1
status: "active"
owners: []
triggers: ["fault injection","allocation failure","timeout","recovery"]
prerequisites: ["test-strategy","debugging-investigation"]
related_docs: ["TESTING.md","docs/audit/2026-07-16.md"]
category: "Quality, release, and docs"
conditional_skills: []
implementation_gates: []
related_milestones: ["M0 Reproducible Build","M7 Beta"]
...
description: "Use when working on fault injection, allocation failure, timeout; provides the FinnOS-specific reliability and fault injection workflow and evidence gates."
---
# Reliability and Fault Injection

## 1. Name

`reliability-fault-injection` (Quality, release, and docs; skill maturity: operational).

## 2. Purpose

Inject deterministic allocation, input, interrupt, timeout, device, storage, network, and service failures.

## 3. When to use this skill

Use for requests mentioning `fault injection`, `allocation failure`, `timeout`, `recovery`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.
...
An evidence-backed reliability and fault injection result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Select one failure boundary and deterministic trigger
2. State pre/post ownership invariants
3. Inject before/after each commit point
4. Verify bounded diagnostics, rollback, and liveness
5. Keep production path unchanged when injection is disabled
...
Preserve the boundary described by the current state: Current tests cover allocator exhaustion and selected expected faults; no general injection framework or recovery tests exist. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Keep production path unchanged when injection is disabled` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Select one failure boundary and deterministic trigger` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: Current tests cover allocator exhaustion and selected expected faults; no general injection framework or recovery tests exist.
...
Request: "Work on fault injection." Correct response: begin by `select one failure boundary and deterministic trigger`, then `state pre/post ownership invariants`, and require evidence for `keep production path unchanged when injection is disabled` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
