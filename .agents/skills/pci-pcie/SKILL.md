...
status: "active"
owners: []
triggers: ["PCI","PCIe","BAR","MSI","ECAM"]
prerequisites: ["acpi-device-discovery","unsafe-rust-low-level-safety","reliability-fault-injection"]
related_docs: ["HARDWARE_SUPPORT.md","ROADMAP.md"]
category: "Drivers and hardware"
conditional_skills: ["driver-architecture","interrupt-exception-handling"]
implementation_gates: ["Requires validated discovery and device IRQ/resource ownership."]
...
description: "Use when working on PCI, PCIe, BAR; provides the FinnOS-specific pci and pcie workflow and evidence gates."
---
# PCI and PCIe

## 1. Name

`pci-pcie` (Drivers and hardware; skill maturity: planning-gated).

## 2. Purpose

Enumerate buses, validate configuration/BARs/capabilities, route MSI, and match drivers safely.

## 3. When to use this skill

Use for requests mentioning `PCI`, `PCIe`, `BAR`, `MSI`, `ECAM`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.

## 4. When not to use this skill

Requires validated discovery and device IRQ/resource ownership. Do not load it only because a future FinnOS document mentions the subsystem.
...
- interrupt-exception-handling for MSI/MSI-X
- driver-architecture for matching/resource grants

Implementation gates are roadmap/runtime conditions, not additional documents to load automatically:

- Requires validated discovery and device IRQ/resource ownership.
...
An evidence-backed pci and pcie result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Obtain validated MCFG or legacy access policy
2. Enumerate bounded bus/device/function space
3. Size/assign BARs without clobbering firmware resources
4. Parse capability lists with loop bounds
5. Test QEMU devices, hot/error paths, and MSI teardown
...
Preserve the boundary described by the current state: No PCI enumeration, ECAM parser, BAR allocator, MSI/MSI-X, or driver matching exists. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Test QEMU devices, hot/error paths, and MSI teardown` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Obtain validated MCFG or legacy access policy` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: No PCI enumeration, ECAM parser, BAR allocator, MSI/MSI-X, or driver matching exists.
...
- Do not disregard the roadmap/status gate: Requires validated discovery and device IRQ/resource ownership.
...
Request: "Work on PCI." Correct response: begin by `obtain validated mcfg or legacy access policy`, then `enumerate bounded bus/device/function space`, and require evidence for `test qemu devices, hot/error paths, and msi teardown` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
