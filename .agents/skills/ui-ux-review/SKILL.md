...
status: "active"
owners: []
triggers: ["UI review","accessibility","visual regression","usability"]
prerequisites: ["test-strategy","documentation-maintenance"]
related_docs: ["UI_GUIDELINES.md",".agents/checklists/ui-review.md"]
category: "Graphics and Peony"
conditional_skills: []
implementation_gates: []
related_milestones: ["M5 Graphical Stack","M7 Beta"]
...
description: "Use when working on UI review, accessibility, visual regression; provides the FinnOS-specific ui/ux review workflow and evidence gates."
---
# UI/UX Review

## 1. Name

`ui-ux-review` (Graphics and Peony; skill maturity: operational).

## 2. Purpose

Apply measurable visual, interaction, accessibility, localization, scaling, state, and performance review gates.

## 3. When to use this skill

Use for requests mentioning `UI review`, `accessibility`, `visual regression`, `usability`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.
...
An evidence-backed ui/ux review result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Capture target/state matrix
2. Measure alignment/type/spacing/contrast
3. Traverse every action by keyboard
4. Inspect accessibility semantics and responsive states
5. Record screenshots, latency, localization, scaling, and usability results
...
Preserve the boundary described by the current state: No implemented UI can currently pass review; this skill also reviews design proposals without relabeling them implemented. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Record screenshots, latency, localization, scaling, and usability results` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Capture target/state matrix` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: No implemented UI can currently pass review; this skill also reviews design proposals without relabeling them implemented.
...
Request: "Work on UI review." Correct response: begin by `capture target/state matrix`, then `measure alignment/type/spacing/contrast`, and require evidence for `record screenshots, latency, localization, scaling, and usability results` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
