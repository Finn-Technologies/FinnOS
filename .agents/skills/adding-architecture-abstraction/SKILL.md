---
name: "adding-architecture-abstraction"
title: "Adding an Architecture Abstraction"
version: 1
status: "active"
owners: []
triggers: ["add abstraction","platform trait"]
prerequisites: ["cross-architecture-design","architecture-documentation-adrs","test-strategy","agent-handoff"]
related_docs: ["ARCHITECTURE.md","PORTING.md"]
...
description: "Use when working on add abstraction, platform trait; provides the FinnOS-specific adding an architecture abstraction workflow and evidence gates."
---
# Adding an Architecture Abstraction

## 1. Name

`adding-architecture-abstraction` (Execution workflows; skill maturity: operational).

## 2. Purpose

Introduce a minimal shared contract only after mapping existing x86 and required ARM semantics.

## 3. When to use this skill

Use for requests mentioning `add abstraction`, `platform trait`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.
...
An evidence-backed adding an architecture abstraction result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Document current invariant/callers
2. Map x86 mechanism
3. Map ARM mechanism from architecture specs
4. Define narrow semantic contract
5. Add contract tests and staged migration
6. Record ADR and intentional differences
...
Preserve the boundary described by the current state: Architecture-independent policy exists but executable dispatch is x86-only, making premature abstractions especially risky. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Record ADR and intentional differences` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Document current invariant/callers` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: Architecture-independent policy exists but executable dispatch is x86-only, making premature abstractions especially risky.
...
Request: "Work on add abstraction." Correct response: begin by `document current invariant/callers`, then `map x86 mechanism`, and require evidence for `record adr and intentional differences` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
