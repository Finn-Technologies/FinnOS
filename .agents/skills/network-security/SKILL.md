...
status: "active"
owners: []
triggers: ["firewall","TLS","socket permission","network threat"]
prerequisites: ["threat-modelling","capability-privilege-review","reliability-fault-injection"]
related_docs: ["SECURITY.md","ROADMAP.md"]
category: "Networking"
conditional_skills: ["cryptography-entropy"]
...
description: "Use when working on firewall, TLS, socket permission; provides the FinnOS-specific network security workflow and evidence gates."
---
# Network Security

## 1. Name

`network-security` (Networking; skill maturity: operational).

## 2. Purpose

Threat-model packet parsing, exhaustion, socket authority, firewalling, entropy, TLS, and service isolation.

## 3. When to use this skill

Use for requests mentioning `firewall`, `TLS`, `socket permission`, `network threat`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.
...
- cryptography-entropy before TLS or signed network identity
...
An evidence-backed network security result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Define external and local attack surfaces
2. Bound every parser/table/queue/timer
3. Require socket/network capabilities
4. Specify firewall default and audit events
5. Gate TLS on reviewed entropy/crypto/trust-store services
...
Preserve the boundary described by the current state: No network exposure exists; entropy, TLS, firewall, socket permissions, and trust store are absent. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Gate TLS on reviewed entropy/crypto/trust-store services` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Define external and local attack surfaces` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: No network exposure exists; entropy, TLS, firewall, socket permissions, and trust store are absent.
...
Request: "Work on firewall." Correct response: begin by `define external and local attack surfaces`, then `bound every parser/table/queue/timer`, and require evidence for `gate tls on reviewed entropy/crypto/trust-store services` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
