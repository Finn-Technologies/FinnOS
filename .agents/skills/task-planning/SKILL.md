...
related_docs: ["ROADMAP.md",".agents/templates/task-plan-template.md"]
...
`task-planning` (Foundations; skill maturity: operational).

## 2. Purpose

Convert a request into bounded work with dependencies, non-goals, acceptance, and verification.

## 3. When to use this skill

Use for requests mentioning `plan task`, `acceptance criteria`, `scope`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.
...
An evidence-backed task planning result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Restate observed problem
2. Select minimum skills
3. Trace dependencies and active overlap
4. Define measurable acceptance and commands
5. Name non-goals, rollback, docs, and blockers
...
Make `Name non-goals, rollback, docs, and blockers` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Restate observed problem` before relying on downstream assumptions.
...
Request: "Work on plan task." Correct response: begin by `restate observed problem`, then `select minimum skills`, and require evidence for `name non-goals, rollback, docs, and blockers` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
