...
status: "active"
owners: []
triggers: ["handoff","end session"]
prerequisites: ["finnos-operating-rules"]
related_docs: [".agents/templates/handoff-template.md",".agents/README.md"]
...
description: "Use when working on handoff, end session; provides the FinnOS-specific agent handoff workflow and evidence gates."
---
# Agent Handoff

## 1. Name

`agent-handoff` (Execution workflows; skill maturity: operational).

## 2. Purpose

Leave durable objective, state, work, files, evidence, unknowns, blockers, risks, Git state, and next skills.

## 3. When to use this skill

Use for requests mentioning `handoff`, `end session`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.
...
An evidence-backed agent handoff result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Record starting and final Git state
2. List work/files without claiming integration
3. List exact commands/results and evidence class
4. Name unknowns/blockers/risks
5. Recommend one next action and required skills
...
Preserve the boundary described by the current state: Conversational memory is not durable; repository-local handoffs must distinguish worktree changes from integrated behavior. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Recommend one next action and required skills` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Record starting and final Git state` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: Conversational memory is not durable; repository-local handoffs must distinguish worktree changes from integrated behavior.
...
Request: "Work on handoff." Correct response: begin by `record starting and final git state`, then `list work/files without claiming integration`, and require evidence for `recommend one next action and required skills` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
