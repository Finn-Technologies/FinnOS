...
related_docs: ["BUILDING.md","rust-toolchain.toml","tools/finnlib/toolchain.py"]
...
`build-environment-management` (Foundations; skill maturity: operational).

## 2. Purpose

Provision and capture a reproducible FinnOS host toolchain without hiding host differences.

## 3. When to use this skill

Use for requests mentioning `toolchain`, `OVMF`, `AAVMF`, `cross compiler`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.
...
An evidence-backed build environment management result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Run doctor before installation
2. Capture host/Rust/Python/QEMU/firmware versions
3. Install only documented targets/tools
4. Distinguish macOS hdiutil from Linux mtools/dosfstools
5. Re-run doctor and record unresolved dependencies
...
Make `Re-run doctor and record unresolved dependencies` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Run doctor before installation` before relying on downstream assumptions.
...
Request: "Work on toolchain." Correct response: begin by `run doctor before installation`, then `capture host/rust/python/qemu/firmware versions`, and require evidence for `re-run doctor and record unresolved dependencies` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
