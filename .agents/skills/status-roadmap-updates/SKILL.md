---
name: "status-roadmap-updates"
title: "Status and Roadmap Updates"
version: 1
status: "active"
owners: []
triggers: ["roadmap update","status percentage","milestone complete"]
prerequisites: ["evidence-status-reporting","roadmap-execution","documentation-maintenance"]
related_docs: ["STATUS.md","ROADMAP.md","docs/audit/2026-07-16.md"]
...
description: "Use when working on roadmap update, status percentage, milestone complete; provides the FinnOS-specific status and roadmap updates workflow and evidence gates."
---
# Status and Roadmap Updates

## 1. Name

`status-roadmap-updates` (Quality, release, and docs; skill maturity: operational).

## 2. Purpose

Change percentages, confidence, milestone state, blockers, and critical path only from acceptance evidence.

## 3. When to use this skill

Use for requests mentioning `roadmap update`, `status percentage`, `milestone complete`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.
...
An evidence-backed status and roadmap updates result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Identify exact claim and acceptance criterion
2. Gather integrated/runtime/hardware evidence
3. Update classification/confidence before percentage
4. Move milestone only when all exit criteria pass
5. Retain historical audit and record blockers/unknowns
...
Preserve the boundary described by the current state: STATUS and ROADMAP are audit-derived; M0 is current and later feature layers remain unimplemented. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Retain historical audit and record blockers/unknowns` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Identify exact claim and acceptance criterion` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: STATUS and ROADMAP are audit-derived; M0 is current and later feature layers remain unimplemented.
...
Request: "Work on roadmap update." Correct response: begin by `identify exact claim and acceptance criterion`, then `gather integrated/runtime/hardware evidence`, and require evidence for `retain historical audit and record blockers/unknowns` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
