...
status: "active"
owners: []
triggers: ["NIC","VirtIO net","packet DMA","offload"]
...
implementation_gates: ["Requires VirtIO/resource broker and a bounded network buffer contract."]
...
description: "Use when working on NIC, VirtIO net, packet DMA; provides the FinnOS-specific networking drivers workflow and evidence gates."
---
# Networking Drivers

## 1. Name

`networking-drivers` (Drivers and hardware; skill maturity: planning-gated).

## 2. Purpose

Define NIC packet ownership, queues, DMA, interrupts, offloads, backpressure, reset, and statistics.

## 3. When to use this skill

Use for requests mentioning `NIC`, `VirtIO net`, `packet DMA`, `offload`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.

## 4. When not to use this skill

Requires VirtIO/resource broker and a bounded network buffer contract. Do not load it only because a future FinnOS document mentions the subsystem.
...
- virtio and driver-architecture for the first NIC

Implementation gates are roadmap/runtime conditions, not additional documents to load automatically:

- Requires VirtIO/resource broker and a bounded network buffer contract.
...
An evidence-backed networking drivers result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Define bounded RX/TX buffer lifecycle
2. Implement VirtIO net with minimal negotiated features
3. Validate lengths/checksum metadata
4. Handle queue backpressure/reset/interrupt moderation
5. Test drops, malformed descriptors, saturation, and restart
...
Preserve the boundary described by the current state: No NIC interface or driver exists; QEMU networking is disabled. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Test drops, malformed descriptors, saturation, and restart` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Define bounded RX/TX buffer lifecycle` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: No NIC interface or driver exists; QEMU networking is disabled.
...
- Do not disregard the roadmap/status gate: Requires VirtIO/resource broker and a bounded network buffer contract.
...
Request: "Work on NIC." Correct response: begin by `define bounded rx/tx buffer lifecycle`, then `implement virtio net with minimal negotiated features`, and require evidence for `test drops, malformed descriptors, saturation, and restart` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
