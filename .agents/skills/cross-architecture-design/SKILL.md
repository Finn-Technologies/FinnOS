---
name: "cross-architecture-design"
title: "Cross-Architecture Design"
version: 1
status: "active"
owners: []
triggers: ["architecture abstraction","platform interface","parity"]
prerequisites: ["task-planning"]
related_docs: ["ARCHITECTURE.md","PORTING.md","kernel/src/arch/mod.rs"]
...
description: "Use when working on architecture abstraction, platform interface, parity; provides the FinnOS-specific cross-architecture design workflow and evidence gates."
---
# Cross-Architecture Design

## 1. Name

`cross-architecture-design` (Boot and kernel; skill maturity: operational).

## 2. Purpose

Separate semantic policy from x86/ARM mechanisms and define parity contracts without premature abstraction.

## 3. When to use this skill

Use for requests mentioning `architecture abstraction`, `platform interface`, `parity`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.
...
An evidence-backed cross-architecture design result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Extract existing semantic invariant from source
2. Map independently to x86 and ARM mechanisms
3. Choose the smallest shared contract
4. Document intentional differences
5. Add contract tests before migrating callers
...
Avoid both x86-shaped traits and lowest-common-denominator APIs; retain target-specific types behind explicit adapters.
...
Preserve the boundary described by the current state: Shared code covers protocol, memory classification/allocation, heap policy, tasks, and interrupt depth; executable architecture dispatch is x86-only. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Add contract tests before migrating callers` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Extract existing semantic invariant from source` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: Shared code covers protocol, memory classification/allocation, heap policy, tasks, and interrupt depth; executable architecture dispatch is x86-only.
...
Request: "Work on architecture abstraction." Correct response: begin by `extract existing semantic invariant from source`, then `map independently to x86 and arm mechanisms`, and require evidence for `add contract tests before migrating callers` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
