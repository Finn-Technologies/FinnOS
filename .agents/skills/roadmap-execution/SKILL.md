...
status: "active"
owners: []
triggers: ["roadmap","milestone","critical path"]
prerequisites: ["task-planning","evidence-status-reporting"]
related_docs: ["ROADMAP.md","STATUS.md","docs/github-planning/issues.yml"]
...
description: "Use when working on roadmap, milestone, critical path; provides the FinnOS-specific roadmap execution workflow and evidence gates."
---
# Roadmap Execution

## 1. Name

`roadmap-execution` (Foundations; skill maturity: operational).

## 2. Purpose

Select and advance critical-path work without premature later-phase implementation.

## 3. When to use this skill

Use for requests mentioning `roadmap`, `milestone`, `critical path`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.
...
An evidence-backed roadmap execution result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Identify current maturity gate
2. Confirm predecessor evidence
3. Split one reviewable outcome
4. Map acceptance to issue and tests
5. Update state only after evidence
...
Preserve the boundary described by the current state: M0 is current; R1 is locally verified pending CI/integration, while R2 and active preemption-context review are immediate priorities. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Update state only after evidence` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Identify current maturity gate` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: M0 is current; R1 is locally verified pending CI/integration, while R2 and active preemption-context review are immediate priorities.
...
Request: "Work on roadmap." Correct response: begin by `identify current maturity gate`, then `confirm predecessor evidence`, and require evidence for `update state only after evidence` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
