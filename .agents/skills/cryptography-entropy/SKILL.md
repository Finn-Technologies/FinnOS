...
- Implementation waits for platform entropy devices and userspace isolation; never create custom cryptography.
...
An evidence-backed cryptography and entropy result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Threat-model boot/platform entropy
2. Use reviewed standard primitives/libraries only
3. Separate entropy collection, DRBG, and consumers
4. Define key generation/storage/rotation/zeroization
5. Test known vectors, failure-to-seed, fork/process separation, and platform variance
...
- [ ] The workflow proves `Threat-model boot/platform entropy` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: No entropy source, RNG, crypto API, key store, signatures, or TLS exists.
...
- Do not disregard the roadmap/status gate: Implementation waits for platform entropy devices and userspace isolation; never create custom cryptography.
