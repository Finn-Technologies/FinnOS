---
name: "implementing-roadmap-issue"
title: "Implementing a Roadmap Issue"
version: 1
status: "active"
owners: []
triggers: ["implement issue","roadmap item"]
prerequisites: ["roadmap-execution","task-planning","test-strategy","documentation-maintenance","git-commit-hygiene","agent-handoff"]
related_docs: ["ROADMAP.md",".agents/templates/task-plan-template.md",".agents/templates/implementation-report-template.md"]
...
description: "Use when working on implement issue, roadmap item; provides the FinnOS-specific implementing a roadmap issue workflow and evidence gates."
---
# Implementing a Roadmap Issue

## 1. Name

`implementing-roadmap-issue` (Execution workflows; skill maturity: operational).

## 2. Purpose

Execute one issue from selection through baseline, code, evidence, docs, commit preparation, and handoff.

## 3. When to use this skill

Use for requests mentioning `implement issue`, `roadmap item`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.
...
- `ROADMAP.md`
- `.agents/templates/task-plan-template.md`
- `.agents/templates/implementation-report-template.md`
...
An evidence-backed implementing a roadmap issue result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Confirm issue/milestone/dependencies
2. Write plan and failing baseline
3. Implement smallest acceptance slice
4. Run narrow and aggregate gates
5. Update docs/status, inspect diff, prepare PR evidence and handoff
...
Preserve the boundary described by the current state: Roadmap R1/R2 are first; active #16/#17 overlap must be checked before scheduler work. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Update docs/status, inspect diff, prepare PR evidence and handoff` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Confirm issue/milestone/dependencies` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: Roadmap R1/R2 are first; active #16/#17 overlap must be checked before scheduler work.
...
Request: "Work on implement issue." Correct response: begin by `confirm issue/milestone/dependencies`, then `write plan and failing baseline`, and require evidence for `update docs/status, inspect diff, prepare pr evidence and handoff` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
