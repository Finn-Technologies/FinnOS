---
name: "core-graphical-applications"
title: "Core Graphical Applications"
version: 1
status: "active"
owners: []
triggers: ["terminal app","file manager","settings app","installer"]
prerequisites: ["peony-toolkit-development","ui-ux-review","test-strategy"]
related_docs: ["UI_GUIDELINES.md","docs/development/application-development.md","ROADMAP.md"]
category: "Graphics and Peony"
conditional_skills: ["package-update-architecture","shell-core-utilities","vfs-filesystems"]
implementation_gates: ["Each app is blocked by its named service and toolkit dependencies; mockups are not implementation."]
related_milestones: ["M5 Graphical Stack","M6 Developer Preview"]
...
description: "Use when working on terminal app, file manager, settings app; provides the FinnOS-specific core graphical applications workflow and evidence gates."
---
# Core Graphical Applications

## 1. Name

`core-graphical-applications` (Graphics and Peony; skill maturity: planning-gated).

## 2. Purpose

Build terminal, file manager, settings, viewer/editor, monitor, and later installer behind explicit dependency gates.

## 3. When to use this skill

Use for requests mentioning `terminal app`, `file manager`, `settings app`, `installer`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.

## 4. When not to use this skill

Each app is blocked by its named service and toolkit dependencies; mockups are not implementation. Do not load it only because a future FinnOS document mentions the subsystem.
...
- shell-core-utilities for terminal
- vfs-filesystems for file manager
- package-update-architecture only for installer/package UI

Implementation gates are roadmap/runtime conditions, not additional documents to load automatically:

- Each app is blocked by its named service and toolkit dependencies; mockups are not implementation.
...
An evidence-backed core graphical applications result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Verify toolkit/runtime/package API is usable
2. Implement terminal after shell/process PTY-equivalent
3. Implement file manager after VFS
4. Implement settings/monitor from bounded service APIs
5. Defer installer until persistent image/recovery exists
...
Preserve the boundary described by the current state: No graphical application or application SDK exists. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Defer installer until persistent image/recovery exists` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Verify toolkit/runtime/package API is usable` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: No graphical application or application SDK exists.
...
- Do not disregard the roadmap/status gate: Each app is blocked by its named service and toolkit dependencies; mockups are not implementation.
...
Request: "Work on terminal app." Correct response: begin by `verify toolkit/runtime/package api is usable`, then `implement terminal after shell/process pty-equivalent`, and require evidence for `defer installer until persistent image/recovery exists` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
