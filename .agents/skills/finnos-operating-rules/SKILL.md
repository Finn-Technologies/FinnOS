...
prerequisites: []
related_docs: ["STATUS.md","ROADMAP.md","ARCHITECTURE.md"]
...
`finnos-operating-rules` (Foundations; skill maturity: operational).
...
Use for requests mentioning `any FinnOS task`, `universal rules`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.
...
- None; this is the root policy skill.
...
An evidence-backed finnos operating rules result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Read .agents/STATE.md and authority order
2. Classify every claim by executed evidence
3. Stop at named roadmap dependencies
4. Preserve commands/logs and leave a handoff
...
Make `Preserve commands/logs and leave a handoff` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Read .agents/STATE.md and authority order` before relying on downstream assumptions.
...
Request: "Work on any FinnOS task." Correct response: begin by `read .agents/state.md and authority order`, then `classify every claim by executed evidence`, and require evidence for `preserve commands/logs and leave a handoff` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
