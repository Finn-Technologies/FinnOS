---
name: "investigating-architectural-change"
title: "Investigating an Architectural Change"
version: 1
status: "active"
owners: []
triggers: ["architecture investigation","design proposal"]
prerequisites: ["repository-orientation","cross-architecture-design","threat-modelling","architecture-documentation-adrs","agent-handoff"]
related_docs: ["docs/proposals/adr/",".agents/templates/investigation-template.md"]
...
description: "Use when working on architecture investigation, design proposal; provides the FinnOS-specific investigating an architectural change workflow and evidence gates."
---
# Investigating an Architectural Change

## 1. Name

`investigating-architectural-change` (Execution workflows; skill maturity: operational).

## 2. Purpose

Produce an evidence-based recommendation with invariants, alternatives, cross-architecture/security impact, prototype, and migration.

## 3. When to use this skill

Use for requests mentioning `architecture investigation`, `design proposal`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.
...
- `docs/proposals/adr/`
- `.agents/templates/investigation-template.md`
...
An evidence-backed investigating an architectural change result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Trace existing behavior/history
2. State constraints/invariants
3. Compare at least viable alternatives
4. Map x86/ARM/security/compatibility
5. Define smallest prototype and falsification criteria
6. Recommend ADR, defer, or reject
...
Preserve the boundary described by the current state: Many durable interfaces remain unresolved; ADRs must not turn planned architecture into implementation claims. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Recommend ADR, defer, or reject` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Trace existing behavior/history` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: Many durable interfaces remain unresolved; ADRs must not turn planned architecture into implementation claims.
...
Request: "Work on architecture investigation." Correct response: begin by `trace existing behavior/history`, then `state constraints/invariants`, and require evidence for `recommend adr, defer, or reject` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
