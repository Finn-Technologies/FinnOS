---
name: "cpu-early-initialization"
title: "CPU and Early Initialization"
version: 1
status: "active"
owners: []
triggers: ["early init","BSP","SMP","CPU features"]
prerequisites: ["cross-architecture-design","logging-diagnostics","unsafe-rust-low-level-safety"]
related_docs: ["kernel/src/bin/x86_64.rs","docs/architecture/boot.md","kernel/docs/invariants.md"]
...
description: "Use when working on early init, BSP, SMP; provides the FinnOS-specific cpu and early initialization workflow and evidence gates."
---
# CPU and Early Initialization

## 1. Name

`cpu-early-initialization` (Boot and kernel; skill maturity: operational).

## 2. Purpose

Maintain initialization ordering before heap, interrupts, and secondary CPUs are safe.

## 3. When to use this skill

Use for requests mentioning `early init`, `BSP`, `SMP`, `CPU features`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.
...
An evidence-backed cpu and early initialization result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Write an initialization dependency graph
2. Mark heap/interrupt/CPU-local availability at each stage
3. Fail through allocation-free serial/panic paths
4. Initialize exception readiness before risky dereference
5. Add secondary-CPU protocol only after BSP contracts stabilize
...
Preserve the boundary described by the current state: Only the x86 BSP initializes; early serial, exception state, memory, paging, heap, timer, scheduler occur in a fixed sequence; no SMP. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Add secondary-CPU protocol only after BSP contracts stabilize` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Write an initialization dependency graph` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: Only the x86 BSP initializes; early serial, exception state, memory, paging, heap, timer, scheduler occur in a fixed sequence; no SMP.
...
Request: "Work on early init." Correct response: begin by `write an initialization dependency graph`, then `mark heap/interrupt/cpu-local availability at each stage`, and require evidence for `add secondary-cpu protocol only after bsp contracts stabilize` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
