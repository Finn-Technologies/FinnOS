---
name: "architecture-documentation-adrs"
title: "Architecture Documentation and ADRs"
version: 1
status: "active"
owners: []
triggers: ["ADR","RFC","architecture decision"]
prerequisites: ["documentation-maintenance","cross-architecture-design","threat-modelling"]
related_docs: ["docs/proposals/adr/README.md","docs/proposals/rfcs/README.md",".agents/templates/architecture-decision-template.md"]
...
description: "Use when working on ADR, RFC, architecture decision; provides the FinnOS-specific architecture documentation and adrs workflow and evidence gates."
---
# Architecture Documentation and ADRs

## 1. Name

`architecture-documentation-adrs` (Quality, release, and docs; skill maturity: operational).

## 2. Purpose

Record durable decisions, alternatives, architecture/security impact, migration, and supersession.

## 3. When to use this skill

Use for requests mentioning `ADR`, `RFC`, `architecture decision`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.
...
- `docs/proposals/adr/README.md`
- `docs/proposals/rfcs/README.md`
- `.agents/templates/architecture-decision-template.md`
...
An evidence-backed architecture documentation and adrs result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Prove the decision is durable/cross-cutting
2. Document problem/evidence/constraints/options
3. Map x86 and ARM plus security
4. Specify migration/rollback/contract tests
5. Mark accepted/rejected/superseded explicitly
...
Preserve the boundary described by the current state: ADRs 0001-0014 capture current direction and x86 milestones; RFC infrastructure exists with no accepted non-template RFC. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Mark accepted/rejected/superseded explicitly` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Prove the decision is durable/cross-cutting` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: ADRs 0001-0014 capture current direction and x86 milestones; RFC infrastructure exists with no accepted non-template RFC.
...
Request: "Work on ADR." Correct response: begin by `prove the decision is durable/cross-cutting`, then `document problem/evidence/constraints/options`, and require evidence for `mark accepted/rejected/superseded explicitly` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
