---
name: "acpi-device-discovery"
title: "ACPI and Device Discovery"
version: 1
status: "active"
owners: []
triggers: ["ACPI","RSDP","MADT","MCFG","FADT"]
prerequisites: ["physical-memory-management","unsafe-rust-low-level-safety","reliability-fault-injection"]
related_docs: ["ARCHITECTURE.md","kernel/src/bin/x86_64.rs","HARDWARE_SUPPORT.md"]
...
description: "Use when working on ACPI, RSDP, MADT; provides the FinnOS-specific acpi and device discovery workflow and evidence gates."
---
# ACPI and Device Discovery

## 1. Name

`acpi-device-discovery` (Drivers and hardware; skill maturity: operational).

## 2. Purpose

Validate firmware tables and derive x86 topology/resources without trusting addresses or checksums.

## 3. When to use this skill

Use for requests mentioning `ACPI`, `RSDP`, `MADT`, `MCFG`, `FADT`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.
...
An evidence-backed acpi and device discovery result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Validate RSDP signature/checksum/revision
2. Bound and map RSDT/XSDT/table headers
3. Parse MADT first, then MCFG/FADT as needed
4. Reject overlap/overflow/duplicate resources
5. Use synthetic fixtures and QEMU table evidence
...
ACPI is primary for x86; ARM64 may use ACPI or device tree only after an explicit firmware policy.
...
Preserve the boundary described by the current state: The loader passes an RSDP address; the kernel maps one page but parses no ACPI table. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Use synthetic fixtures and QEMU table evidence` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Validate RSDP signature/checksum/revision` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: The loader passes an RSDP address; the kernel maps one page but parses no ACPI table.
...
Request: "Work on ACPI." Correct response: begin by `validate rsdp signature/checksum/revision`, then `bound and map rsdt/xsdt/table headers`, and require evidence for `use synthetic fixtures and qemu table evidence` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
