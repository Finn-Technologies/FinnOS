---
name: "udp-tcp-dhcp-dns"
title: "UDP, TCP, DHCP, and DNS"
version: 1
status: "active"
owners: []
triggers: ["UDP","TCP","DHCP","DNS"]
prerequisites: ["ethernet-arp-ipv4-icmp","timer-timekeeping","network-security","reliability-fault-injection"]
related_docs: ["ROADMAP.md","SECURITY.md"]
category: "Networking"
conditional_skills: []
implementation_gates: ["Requires base IPv4 path; TCP requires mature timer, buffering, and resource-limit design."]
related_milestones: ["M4 Devices and Storage","M7 Beta"]
...
description: "Use when working on UDP, TCP, DHCP; provides the FinnOS-specific udp, tcp, dhcp, and dns workflow and evidence gates."
---
# UDP, TCP, DHCP, and DNS

## 1. Name

`udp-tcp-dhcp-dns` (Networking; skill maturity: planning-gated).

## 2. Purpose

Implement bounded transport/configuration/resolution state machines with timers and interoperability evidence.

## 3. When to use this skill

Use for requests mentioning `UDP`, `TCP`, `DHCP`, `DNS`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.

## 4. When not to use this skill

Requires base IPv4 path; TCP requires mature timer, buffering, and resource-limit design. Do not load it only because a future FinnOS document mentions the subsystem.
...
- Requires base IPv4 path; TCP requires mature timer, buffering, and resource-limit design.
...
An evidence-backed udp, tcp, dhcp, and dns result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Add UDP demux/queues/checksums and limits
2. Add DHCP lease state with hostile-option bounds
3. Add DNS parser/cache/retry limits
4. Implement TCP state/retransmit/window/congestion scope only after timer tests
5. Fuzz and interoperate each layer independently
...
Preserve the boundary described by the current state: None of UDP, TCP, DHCP, or DNS exists; roadmap first network scope names UDP/DHCP/DNS before TCP/IPv6. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Fuzz and interoperate each layer independently` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Add UDP demux/queues/checksums and limits` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: None of UDP, TCP, DHCP, or DNS exists; roadmap first network scope names UDP/DHCP/DNS before TCP/IPv6.
...
- Do not disregard the roadmap/status gate: Requires base IPv4 path; TCP requires mature timer, buffering, and resource-limit design.
...
Request: "Work on UDP." Correct response: begin by `add udp demux/queues/checksums and limits`, then `add dhcp lease state with hostile-option bounds`, and require evidence for `fuzz and interoperate each layer independently` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
