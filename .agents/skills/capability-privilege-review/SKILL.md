---
name: "capability-privilege-review"
title: "Capability and Privilege Review"
version: 1
status: "active"
owners: []
triggers: ["least privilege","capability review","rights"]
prerequisites: ["threat-modelling"]
related_docs: ["docs/architecture/capabilities.md","SECURITY.md"]
category: "Security"
conditional_skills: []
...
description: "Use when working on least privilege, capability review, rights; provides the FinnOS-specific capability and privilege review workflow and evidence gates."
---
# Capability and Privilege Review

## 1. Name

`capability-privilege-review` (Security; skill maturity: operational).

## 2. Purpose

Verify least authority, rights, transfer, revocation, device/service privilege, and audit evidence.

## 3. When to use this skill

Use for requests mentioning `least privilege`, `capability review`, `rights`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.
...
An evidence-backed capability and privilege review result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. List required authority per operation
2. Remove ambient/global access
3. Trace handle grant/duplicate/transfer/revoke
4. Review service and driver resource scope
5. Test forged/stale/excess rights and audit events
...
Preserve the boundary described by the current state: Capability architecture is planned only; no enforced rights or service/process boundary exists. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Test forged/stale/excess rights and audit events` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `List required authority per operation` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: Capability architecture is planned only; no enforced rights or service/process boundary exists.
...
Request: "Work on least privilege." Correct response: begin by `list required authority per operation`, then `remove ambient/global access`, and require evidence for `test forged/stale/excess rights and audit events` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
