---
name: "package-update-architecture"
title: "Package and Update Architecture"
version: 1
status: "active"
owners: []
triggers: ["package","update","rollback","repository metadata"]
prerequisites: ["task-planning","threat-modelling","architecture-documentation-adrs"]
related_docs: ["RELEASING.md","SECURITY.md","ROADMAP.md"]
category: "Userspace and platform"
conditional_skills: ["cryptography-entropy","init-service-management","secure-updates-recovery","vfs-filesystems"]
implementation_gates: ["Planning only until persistent storage, process isolation, signing/entropy, and recovery boot paths exist."]
...
description: "Use when working on package, update, rollback; provides the FinnOS-specific package and update architecture workflow and evidence gates."
---
# Package and Update Architecture

## 1. Name

`package-update-architecture` (Userspace and platform; skill maturity: planning).

## 2. Purpose

Plan signed packages, metadata, atomic updates, rollback, recovery, and versions after prerequisites exist.

## 3. When to use this skill

Use for requests mentioning `package`, `update`, `rollback`, `repository metadata`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.

## 4. When not to use this skill

Planning only until persistent storage, process isolation, signing/entropy, and recovery boot paths exist. Do not load it only because a future FinnOS document mentions the subsystem.
...
- vfs-filesystems, init-service-management, cryptography-entropy, and secure-updates-recovery before implementation

Implementation gates are roadmap/runtime conditions, not additional documents to load automatically:

- Planning only until persistent storage, process isolation, signing/entropy, and recovery boot paths exist.
...
An evidence-backed package and update architecture result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Wait for userspace/storage/security foundations
2. Define content-addressed signed metadata and capability manifests
3. Separate app packages from system image updates
4. Specify atomic staging/activation/rollback
5. Threat-model repository, keys, downgrade, and interrupted update
...
Preserve the boundary described by the current state: No package format, repository, updater, persistent root, signing, or recovery exists. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Threat-model repository, keys, downgrade, and interrupted update` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Wait for userspace/storage/security foundations` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: No package format, repository, updater, persistent root, signing, or recovery exists.
...
- Do not disregard the roadmap/status gate: Planning only until persistent storage, process isolation, signing/entropy, and recovery boot paths exist.
...
Request: "Work on package." Correct response: begin by `wait for userspace/storage/security foundations`, then `define content-addressed signed metadata and capability manifests`, and require evidence for `threat-model repository, keys, downgrade, and interrupted update` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
