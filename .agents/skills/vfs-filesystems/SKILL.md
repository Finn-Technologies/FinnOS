---
name: "vfs-filesystems"
title: "VFS and Filesystems"
version: 1
status: "active"
owners: []
triggers: ["VFS","filesystem","mount","inode","file descriptor"]
prerequisites: ["synchronization-concurrency","reliability-fault-injection","threat-modelling"]
related_docs: ["ROADMAP.md","SECURITY.md"]
category: "Drivers and hardware"
conditional_skills: ["ipc-capabilities","storage-block-devices"]
implementation_gates: ["Requires block service, userspace/IPC, and an accepted on-disk compatibility decision."]
...
description: "Use when working on VFS, filesystem, mount; provides the FinnOS-specific vfs and filesystems workflow and evidence gates."
---
# VFS and Filesystems

## 1. Name

`vfs-filesystems` (Drivers and hardware; skill maturity: planning-gated).

## 2. Purpose

Design namespace, mounts, file objects, permissions, caching, concurrency, and crash-consistent persistence.

## 3. When to use this skill

Use for requests mentioning `VFS`, `filesystem`, `mount`, `inode`, `file descriptor`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.

## 4. When not to use this skill

Requires block service, userspace/IPC, and an accepted on-disk compatibility decision. Do not load it only because a future FinnOS document mentions the subsystem.
...
- storage-block-devices and ipc-capabilities during implementation

Implementation gates are roadmap/runtime conditions, not additional documents to load automatically:

- Requires block service, userspace/IPC, and an accepted on-disk compatibility decision.
...
An evidence-backed vfs and filesystems result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Define vnode/file/mount/namespace lifetime
2. Specify path normalization and authority
3. Select a minimal root/persistent format with migration policy
4. Handle cache/writeback/flush/crash ordering
5. Test hostile images, concurrency, power loss, and reboot persistence
...
Preserve the boundary described by the current state: No VFS, runtime filesystem, descriptors, root mount, permissions, or persistent user data exists. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Test hostile images, concurrency, power loss, and reboot persistence` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Define vnode/file/mount/namespace lifetime` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: No VFS, runtime filesystem, descriptors, root mount, permissions, or persistent user data exists.
...
- Do not disregard the roadmap/status gate: Requires block service, userspace/IPC, and an accepted on-disk compatibility decision.
...
Request: "Work on VFS." Correct response: begin by `define vnode/file/mount/namespace lifetime`, then `specify path normalization and authority`, and require evidence for `test hostile images, concurrency, power loss, and reboot persistence` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
