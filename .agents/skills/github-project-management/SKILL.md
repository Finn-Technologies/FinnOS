...
status: "active"
owners: []
triggers: ["GitHub issue","milestone","labels","project"]
prerequisites: ["roadmap-execution","evidence-status-reporting"]
related_docs: ["docs/github-planning/README.md","docs/github-planning/issues.yml",".github/ISSUE_TEMPLATE/"]
...
description: "Use when working on GitHub issue, milestone, labels; provides the FinnOS-specific github project management workflow and evidence gates."
---
# GitHub Project Management

## 1. Name

`github-project-management` (Foundations; skill maturity: operational).

## 2. Purpose

Create evidence-based issues, milestones, labels, PR metadata, and fallback proposals.

## 3. When to use this skill

Use for requests mentioning `GitHub issue`, `milestone`, `labels`, `project`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.
...
- `docs/github-planning/README.md`
- `docs/github-planning/issues.yml`
...
An evidence-backed github project management result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Search existing issues/PRs first
2. Use roadmap acceptance and dependencies
3. Prefer epics plus independently reviewable children
4. Update actual state from evidence
5. Write machine-readable proposals if write access is absent
...
Preserve the boundary described by the current state: Issue #16/PR #17 are active; proposed normalized metadata lives under docs/github-planning and Projects permission may be unavailable. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Write machine-readable proposals if write access is absent` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Search existing issues/PRs first` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: Issue #16/PR #17 are active; proposed normalized metadata lives under docs/github-planning and Projects permission may be unavailable.
...
Request: "Work on GitHub issue." Correct response: begin by `search existing issues/prs first`, then `use roadmap acceptance and dependencies`, and require evidence for `write machine-readable proposals if write access is absent` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
