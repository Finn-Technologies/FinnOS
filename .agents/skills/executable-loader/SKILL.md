...
status: "active"
owners: []
triggers: ["user ELF","executable loader","process image"]
prerequisites: ["userspace-isolation","virtual-memory","unsafe-rust-low-level-safety"]
related_docs: ["boot/uefi/src/lib.rs","docs/architecture/processes.md"]
category: "Userspace and platform"
conditional_skills: []
implementation_gates: ["Requires user address spaces and syscall/process lifecycle."]
...
description: "Use when working on user ELF, executable loader, process image; provides the FinnOS-specific executable loader workflow and evidence gates."
---
# Executable Loader

## 1. Name

`executable-loader` (Userspace and platform; skill maturity: planning-gated).

## 2. Purpose

Load untrusted user ELF segments, permissions, stack, arguments, and ABI state safely.

## 3. When to use this skill

Use for requests mentioning `user ELF`, `executable loader`, `process image`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.

## 4. When not to use this skill

Requires user address spaces and syscall/process lifecycle. Do not load it only because a future FinnOS document mentions the subsystem.
...
- Requires user address spaces and syscall/process lifecycle.

## 6. Authoritative repository references

- `boot/uefi/src/lib.rs`
...
An evidence-backed executable loader result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Define supported ELF class/machine/type/relocations
2. Validate every range/overlap/permission
3. Create W^X user mappings and guarded stack
4. Populate bounded arguments/environment
5. Test hostile ELF and cleanup after partial failure
...
Preserve the boundary described by the current state: Only the trusted kernel ELF loader exists; no user executable loader or process exists. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Test hostile ELF and cleanup after partial failure` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Define supported ELF class/machine/type/relocations` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: Only the trusted kernel ELF loader exists; no user executable loader or process exists.
...
- Do not disregard the roadmap/status gate: Requires user address spaces and syscall/process lifecycle.
...
Request: "Work on user ELF." Correct response: begin by `define supported elf class/machine/type/relocations`, then `validate every range/overlap/permission`, and require evidence for `test hostile elf and cleanup after partial failure` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
