---
name: "peony-toolkit-development"
title: "Peony Toolkit Development"
version: 1
status: "active"
owners: []
triggers: ["widget","layout","Peony toolkit","accessibility tree"]
prerequisites: ["peony-design-system","text-fonts-localization","ui-ux-review","performance-engineering"]
related_docs: ["UI_GUIDELINES.md","docs/architecture/peony.md"]
category: "Graphics and Peony"
conditional_skills: ["compositor-window-system","runtime-standard-library"]
implementation_gates: ["Requires compositor, text, userspace runtime, and input."]
...
description: "Use when working on widget, layout, Peony toolkit; provides the FinnOS-specific peony toolkit development workflow and evidence gates."
---
# Peony Toolkit Development

## 1. Name

`peony-toolkit-development` (Graphics and Peony; skill maturity: planning-gated).

## 2. Purpose

Build widget tree, layout, painting, events, focus, accessibility, themes, text, invalidation, and lifecycle APIs.

## 3. When to use this skill

Use for requests mentioning `widget`, `layout`, `Peony toolkit`, `accessibility tree`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.

## 4. When not to use this skill

Requires compositor, text, userspace runtime, and input. Do not load it only because a future FinnOS document mentions the subsystem.
...
- compositor-window-system and runtime-standard-library during implementation

Implementation gates are roadmap/runtime conditions, not additional documents to load automatically:

- Requires compositor, text, userspace runtime, and input.
...
An evidence-backed peony toolkit development result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Define retained/immediate state ownership deliberately
2. Implement deterministic layout constraints
3. Propagate invalidation/damage minimally
4. Unify pointer/keyboard/assistive actions
5. Test widgets, accessibility tree, themes, text, lifecycle, and API examples
...
Preserve the boundary described by the current state: No toolkit, widget tree, app lifecycle, accessibility tree, or API stability exists. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Test widgets, accessibility tree, themes, text, lifecycle, and API examples` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Define retained/immediate state ownership deliberately` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: No toolkit, widget tree, app lifecycle, accessibility tree, or API stability exists.
...
- Do not disregard the roadmap/status gate: Requires compositor, text, userspace runtime, and input.
...
Request: "Work on widget." Correct response: begin by `define retained/immediate state ownership deliberately`, then `implement deterministic layout constraints`, and require evidence for `test widgets, accessibility tree, themes, text, lifecycle, and api examples` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
