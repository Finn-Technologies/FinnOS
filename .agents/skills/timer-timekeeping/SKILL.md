---
name: "timer-timekeeping"
title: "Timer and Timekeeping"
version: 1
status: "active"
owners: []
triggers: ["timer","timekeeping","sleep","deadline"]
prerequisites: ["interrupt-exception-handling","cross-architecture-design"]
related_docs: ["kernel/src/arch/x86_64/timer.rs","docs/architecture/x86_64-interrupts-and-timer.md"]
...
description: "Use when working on timer, timekeeping, sleep; provides the FinnOS-specific timer and timekeeping workflow and evidence gates."
---
# Timer and Timekeeping

## 1. Name

`timer-timekeeping` (Boot and kernel; skill maturity: operational).

## 2. Purpose

Evolve hardware timers into monotonic clocks, deadlines, and scheduler time without conflating wall time.

## 3. When to use this skill

Use for requests mentioning `timer`, `timekeeping`, `sleep`, `deadline`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.
...
- `docs/architecture/x86_64-interrupts-and-timer.md`
...
An evidence-backed timer and timekeeping result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Define counter frequency/width/wrap
2. Separate clocksource, event timer, monotonic, and wall clock
3. Validate calibration bounds
4. Integrate deadlines through deferred scheduling
5. Test drift, wrap, missed events, and architecture parity
...
Preserve the boundary described by the current state: x86 calibrates a 100 Hz local APIC timer with PIT channel 2 and increments atomic ticks; no sleep queue, wall clock, tickless mode, or ARM timer. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Test drift, wrap, missed events, and architecture parity` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Define counter frequency/width/wrap` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: x86 calibrates a 100 Hz local APIC timer with PIT channel 2 and increments atomic ticks; no sleep queue, wall clock, tickless mode, or ARM timer.
...
Request: "Work on timer." Correct response: begin by `define counter frequency/width/wrap`, then `separate clocksource, event timer, monotonic, and wall clock`, and require evidence for `test drift, wrap, missed events, and architecture parity` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
