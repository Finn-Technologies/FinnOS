...
status: "active"
owners: []
triggers: ["VirtIO","virtqueue","descriptor","feature negotiation"]
prerequisites: ["synchronization-concurrency","unsafe-rust-low-level-safety","reliability-fault-injection"]
related_docs: ["HARDWARE_SUPPORT.md","ROADMAP.md"]
category: "Drivers and hardware"
conditional_skills: ["driver-architecture","interrupt-exception-handling","pci-pcie"]
implementation_gates: ["Requires driver resource broker, IRQ routing, and DMA policy."]
...
description: "Use when working on VirtIO, virtqueue, descriptor; provides the FinnOS-specific virtio workflow and evidence gates."
---
# VirtIO

## 1. Name

`virtio` (Drivers and hardware; skill maturity: planning-gated).

## 2. Purpose

Implement portable, bounded feature negotiation, virtqueues, barriers, DMA, interrupts, and reset.

## 3. When to use this skill

Use for requests mentioning `VirtIO`, `virtqueue`, `descriptor`, `feature negotiation`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.

## 4. When not to use this skill

Requires driver resource broker, IRQ routing, and DMA policy. Do not load it only because a future FinnOS document mentions the subsystem.
...
- pci-pcie or the accepted MMIO transport
- driver-architecture and interrupt-exception-handling during integration

Implementation gates are roadmap/runtime conditions, not additional documents to load automatically:

- Requires driver resource broker, IRQ routing, and DMA policy.
...
An evidence-backed virtio result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Select PCI/MMIO transport from discovered resources
2. Negotiate only implemented features
3. Validate descriptor chains and ownership
4. Apply device/CPU memory barriers
5. Test reset, malformed chains, queue exhaustion, and interrupt races
...
Preserve the boundary described by the current state: No VirtIO transport or device driver exists; it is the planned first virtual-device family. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Test reset, malformed chains, queue exhaustion, and interrupt races` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Select PCI/MMIO transport from discovered resources` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: No VirtIO transport or device driver exists; it is the planned first virtual-device family.
...
- Do not disregard the roadmap/status gate: Requires driver resource broker, IRQ routing, and DMA policy.
...
Request: "Work on VirtIO." Correct response: begin by `select pci/mmio transport from discovered resources`, then `negotiate only implemented features`, and require evidence for `test reset, malformed chains, queue exhaustion, and interrupt races` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
