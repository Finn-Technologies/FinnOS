...
status: "active"
owners: []
triggers: ["review PR","approve pull request"]
prerequisites: ["code-review","test-strategy","evidence-status-reporting","agent-handoff"]
related_docs: ["CONTRIBUTING.md",".agents/checklists/pre-commit.md"]
...
description: "Use when working on review PR, approve pull request; provides the FinnOS-specific reviewing a pull request workflow and evidence gates."
---
# Reviewing a Pull Request

## 1. Name

`reviewing-pull-request` (Execution workflows; skill maturity: operational).

## 2. Purpose

Verify claims through local reproduction, invariants, unsafe, architecture, security, performance, docs, and scope before approval.

## 3. When to use this skill

Use for requests mentioning `review PR`, `approve pull request`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.
...
An evidence-backed reviewing a pull request result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Read issue/acceptance and all commits
2. Reproduce baseline/change where feasible
3. Audit diff and generated artifacts
4. Challenge unsafe/bounds/ownership/architecture/security
5. Evaluate negative tests and docs
6. Approve only when claims match evidence
...
Preserve the boundary described by the current state: Recent PR #17 has green latest checks but earlier failures and explicitly does not implement timer-ISR preemption. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Approve only when claims match evidence` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Read issue/acceptance and all commits` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: Recent PR #17 has green latest checks but earlier failures and explicitly does not implement timer-ISR preemption.
...
Request: "Work on review PR." Correct response: begin by `read issue/acceptance and all commits`, then `reproduce baseline/change where feasible`, and require evidence for `approve only when claims match evidence` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
