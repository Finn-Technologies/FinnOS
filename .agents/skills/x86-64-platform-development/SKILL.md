---
name: "x86-64-platform-development"
title: "x86-64 Platform Development"
version: 1
status: "active"
owners: []
triggers: ["x86_64","GDT","TSS","APIC","syscall"]
prerequisites: ["cross-architecture-design","cpu-early-initialization","unsafe-rust-low-level-safety"]
related_docs: ["kernel/src/arch/x86_64/","kernel/arch/x86_64/linker.ld","architecture/x86_64/README.md"]
category: "Boot and kernel"
conditional_skills: ["interrupt-exception-handling","scheduler-thread-development","virtual-memory"]
...
description: "Use when working on x86_64, GDT, TSS; provides the FinnOS-specific x86-64 platform development workflow and evidence gates."
---
# x86-64 Platform Development

## 1. Name

`x86-64-platform-development` (Boot and kernel; skill maturity: operational).

## 2. Purpose

Modify x86 entry, descriptor tables, paging, APIC/PIC/PIT, context, ACPI, SMP, and future syscall mechanisms.

## 3. When to use this skill

Use for requests mentioning `x86_64`, `GDT`, `TSS`, `APIC`, `syscall`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.
...
- interrupt-exception-handling for trap/APIC/IRQ work
- virtual-memory for page-table work
- scheduler-thread-development for context work
...
An evidence-backed x86-64 platform development result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Trace linker/entry/init order
2. State required CPU features and register invariants
3. Normalize traps before shared policy
4. Preserve canonical addresses and interrupt ownership
5. Test real QEMU behavior, not assembly compilation only
...
Preserve the boundary described by the current state: x86 has verified BSP entry, GDT/TSS/IDT, private paging, xAPIC timer, and cooperative SysV64 contexts; no IOAPIC, SMP, user mode, or syscall entry. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Test real QEMU behavior, not assembly compilation only` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Trace linker/entry/init order` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: x86 has verified BSP entry, GDT/TSS/IDT, private paging, xAPIC timer, and cooperative SysV64 contexts; no IOAPIC, SMP, user mode, or syscall entry.
...
Request: "Work on x86_64." Correct response: begin by `trace linker/entry/init order`, then `state required cpu features and register invariants`, and require evidence for `test real qemu behavior, not assembly compilation only` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
