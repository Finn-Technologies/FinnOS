...
status: "active"
owners: []
triggers: ["physical memory","page allocator","UEFI map","DMA"]
prerequisites: ["boot-protocol-evolution","cross-architecture-design","reliability-fault-injection"]
related_docs: ["kernel/src/memory/","docs/architecture/physical-memory-map.md","docs/architecture/physical-page-allocation.md"]
...
description: "Use when working on physical memory, page allocator, UEFI map; provides the FinnOS-specific physical memory management workflow and evidence gates."
---
# Physical Memory Management

## 1. Name

`physical-memory-management` (Boot and kernel; skill maturity: operational).

## 2. Purpose

Classify, reserve, allocate, reclaim, and account physical pages with explicit ownership.

## 3. When to use this skill

Use for requests mentioning `physical memory`, `page allocator`, `UEFI map`, `DMA`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.
...
- `kernel/src/memory/`
- `docs/architecture/physical-memory-map.md`
- `docs/architecture/physical-page-allocation.md`
...
An evidence-backed physical memory management result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Validate descriptor stride/count/range/overlap
2. Subtract kernel/handoff/framebuffer/platform reservations
3. Preserve page alignment and ownership
4. Model capacity/exhaustion/double-free
5. Plan reclamation and DMA zones only from measured needs
...
Preserve the boundary described by the current state: UEFI descriptors become bounded normalized regions; only Conventional memory is allocated; boot-service memory is not reclaimed; contiguous first-fit is tested. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Plan reclamation and DMA zones only from measured needs` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Validate descriptor stride/count/range/overlap` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: UEFI descriptors become bounded normalized regions; only Conventional memory is allocated; boot-service memory is not reclaimed; contiguous first-fit is tested.
...
Request: "Work on physical memory." Correct response: begin by `validate descriptor stride/count/range/overlap`, then `subtract kernel/handoff/framebuffer/platform reservations`, and require evidence for `plan reclamation and dma zones only from measured needs` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
