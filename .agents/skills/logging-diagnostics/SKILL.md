...
status: "active"
owners: []
triggers: ["logging","serial","crash log","structured event"]
prerequisites: ["synchronization-concurrency","reliability-fault-injection"]
related_docs: ["docs/development/debugging.md","kernel/docs/panic-policy.md","SECURITY.md"]
category: "Userspace and platform"
conditional_skills: []
implementation_gates: []
related_milestones: ["M0 Reproducible Build","M3 Userspace Foundation","M7 Beta"]
...
description: "Use when working on logging, serial, crash log; provides the FinnOS-specific logging and diagnostics workflow and evidence gates."
---
# Logging and Diagnostics

## 1. Name

`logging-diagnostics` (Userspace and platform; skill maturity: operational).

## 2. Purpose

Unify allocation-safe early serial, bounded kernel events, userspace logging, crash capture, privacy, and tooling.

## 3. When to use this skill

Use for requests mentioning `logging`, `serial`, `crash log`, `structured event`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.
...
An evidence-backed logging and diagnostics result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Define severity/component/event IDs and bounded fields
2. Keep early path allocation-free with timeout policy
3. Add lock/context-safe kernel ring buffer
4. Route userspace logs through explicit capability
5. Redact, rotate, persist, and test crash/log-loss behavior
...
Preserve the boundary described by the current state: Early/kernel COM1 markers work; no ring buffer, structured event schema, userspace collector, persistent logs, or privacy policy exists. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Redact, rotate, persist, and test crash/log-loss behavior` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Define severity/component/event IDs and bounded fields` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: Early/kernel COM1 markers work; no ring buffer, structured event schema, userspace collector, persistent logs, or privacy policy exists.
...
Request: "Work on logging." Correct response: begin by `define severity/component/event ids and bounded fields`, then `keep early path allocation-free with timeout policy`, and require evidence for `redact, rotate, persist, and test crash/log-loss behavior` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
