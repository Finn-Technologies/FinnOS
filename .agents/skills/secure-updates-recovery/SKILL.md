---
name: "secure-updates-recovery"
title: "Secure Updates and Recovery"
version: 1
status: "active"
owners: []
triggers: ["secure update","recovery","rollback protection"]
prerequisites: ["cryptography-entropy","vfs-filesystems","reliability-fault-injection"]
related_docs: ["RELEASING.md","SECURITY.md","ROADMAP.md"]
category: "Security"
conditional_skills: []
implementation_gates: ["Requires persistent storage, release signing, boot policy, and recovery image."]
...
description: "Use when working on secure update, recovery, rollback protection; provides the FinnOS-specific secure updates and recovery workflow and evidence gates."
---
# Secure Updates and Recovery

## 1. Name

`secure-updates-recovery` (Security; skill maturity: planning-gated).

## 2. Purpose

Design signed, atomic, rollback-aware updates and recovery that preserve user data.

## 3. When to use this skill

Use for requests mentioning `secure update`, `recovery`, `rollback protection`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.

## 4. When not to use this skill

Requires persistent storage, release signing, boot policy, and recovery image. Do not load it only because a future FinnOS document mentions the subsystem.
...
- Requires persistent storage, release signing, boot policy, and recovery image.
...
An evidence-backed secure updates and recovery result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Define signed manifest/provenance and trust roots
2. Choose atomic slot/snapshot activation
3. Handle failed boot and power loss
4. Separate rollback recovery from anti-downgrade policy
5. Test tamper, interrupted update, key rotation, and data migration
...
Preserve the boundary described by the current state: No persistent system, updater, signatures, boot slot policy, recovery environment, or rollback protection exists. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Test tamper, interrupted update, key rotation, and data migration` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Define signed manifest/provenance and trust roots` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: No persistent system, updater, signatures, boot slot policy, recovery environment, or rollback protection exists.
...
- Do not disregard the roadmap/status gate: Requires persistent storage, release signing, boot policy, and recovery image.
...
Request: "Work on secure update." Correct response: begin by `define signed manifest/provenance and trust roots`, then `choose atomic slot/snapshot activation`, and require evidence for `test tamper, interrupted update, key rotation, and data migration` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
