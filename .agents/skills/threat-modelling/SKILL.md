...
status: "active"
owners: []
triggers: ["threat model","attack surface","trust boundary"]
prerequisites: ["repository-orientation","evidence-status-reporting"]
related_docs: ["SECURITY.md","docs/audit/2026-07-16.md"]
category: "Security"
conditional_skills: []
implementation_gates: []
related_milestones: ["M2 Core Kernel","M7 Beta"]
...
description: "Use when working on threat model, attack surface, trust boundary; provides the FinnOS-specific threat modelling workflow and evidence gates."
---
# Threat Modelling

## 1. Name

`threat-modelling` (Security; skill maturity: operational).

## 2. Purpose

Identify assets, actors, trust boundaries, attack surfaces, likelihood/impact, mitigations, and review cadence.

## 3. When to use this skill

Use for requests mentioning `threat model`, `attack surface`, `trust boundary`, or when a dependency points to this skill. Load only after repository entry and before design or implementation.
...
An evidence-backed threat modelling result with scoped artifacts, tests, documentation, and handoff. Include acceptance evidence, residual limitations, and next dependency rather than only code.

## 10. Step-by-step workflow

1. Scope assets and supported deployment
2. Draw firmware/loader/kernel/process/driver/service/storage/network/update boundaries
3. Enumerate threats and denial-of-service
4. Prioritize by evidence and milestone
5. Assign mitigations/tests/residual risk and revisit on boundary changes
...
Preserve the boundary described by the current state: SECURITY.md defines a baseline but no complete threat model exists; current ring-0-only system has no hostile-code boundary. Apply `.agents/checklists/pre-change.md`; for kernel, driver, security, architecture, or UI work also apply the matching checklist.

## 14. Testing requirements

Make `Assign mitigations/tests/residual risk and revisit on boundary changes` observable with a negative case, then run the narrow and aggregate gates. Do not permanently hard-code test counts; counts belong to dated evidence reports.
...
- [ ] The workflow proves `Scope assets and supported deployment` before relying on downstream assumptions.
...
- Ignoring this skill's current constraint: SECURITY.md defines a baseline but no complete threat model exists; current ring-0-only system has no hostile-code boundary.
...
Request: "Work on threat model." Correct response: begin by `scope assets and supported deployment`, then `draw firmware/loader/kernel/process/driver/service/storage/network/update boundaries`, and require evidence for `assign mitigations/tests/residual risk and revisit on boundary changes` before changing status. Incorrect response: create a plausible subsystem scaffold and mark the roadmap item complete because it compiles.
