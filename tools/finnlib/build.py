...
def build_boot(root: Path, test: bool = False, exceptions: bool = False, memory_map: bool = False, page_allocator: bool = False, page_tables: bool = False, heap: bool = False) -> tuple[Path, Path]:
    output = root / "build" / "out" / ("x86_64-qemu-heap" if heap else "x86_64-qemu-page-tables" if page_tables else "x86_64-qemu-page-allocator" if page_allocator else "x86_64-qemu-memory-map" if memory_map else "x86_64-qemu-exceptions" if exceptions else "x86_64-qemu-test" if test else "x86_64-qemu")
    ...
    target_dir = root / "target" / ("x86_64-unknown-none-heap" if heap else "x86_64-unknown-none-page-tables" if page_tables else "x86_64-unknown-none-page-allocator" if page_allocator else "x86_64-unknown-none-memory-map" if memory_map else "x86_64-unknown-none-exceptions" if exceptions else "x86_64-unknown-none-test" if test else "x86_64-unknown-none-normal")
