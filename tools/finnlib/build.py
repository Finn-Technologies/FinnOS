...
def build_boot(root: Path, test: bool = False, exceptions: bool = False, memory_map: bool = False, page_allocator: bool = False, page_tables: bool = False, heap: bool = False, timer: bool = False, cooperative_tasks: bool = False, preemption_context: bool = False) -> tuple[Path, Path]:
    ...
    if preemption_context:
        kernel_features = "kernel-bin,qemu-test-preemption-context"
    elif cooperative_tasks:
        kernel_features = "kernel-bin,qemu-test-exit,qemu-test-cooperative-tasks"
    elif timer:
    ...
    target_dir = root / "target" / ("x86_64-unknown-none-preemption-context" if preemption_context else "x86_64-unknown-none-cooperative-tasks" if cooperative_tasks else "x86_64-unknown-none-timer-interrupts" if timer else "x86_64-unknown-none-heap" if heap else "x86_64-unknown-none-page-tables" if page_tables else "x86_64-unknown-none-page-allocator" if page_allocator else "x86_64-unknown-none-memory-map" if memory_map else "x86_64-unknown-none-exceptions" if exceptions else "x86_64-unknown-none-test" if test else "x86_64-unknown-none-normal")
