...
class BootLogTests(unittest.TestCase):
    def test_preemption_context_markers_are_strict(self):
        numeric = {
            ...
            "SOFTWARE_INTERRUPTED_RSP": 0x10b8, "SOFTWARE_EXPECTED_RSP": 0x10b8, "SOFTWARE_POST_RSP": 0x10b8, "SOFTWARE_SAVED_RSP_FIELD": 0x10a0, "SOFTWARE_SAVED_SS": 0x10,
            "TIMER_FRAME": 0x4000, "TIMER_RETURN_FRAME": 0x4000, "TIMER_VECTOR": 0x40, "TIMER_CS": 0x8, "TIMER_RFLAGS": 0x202, "TIMER_SAVED_RIP": 0x5000,
            "TIMER_LOOP_START": 0x4fff, "TIMER_LOOP_END": 0x5001, "TIMER_INTERRUPTED_RSP": 0x40b8,
            "TIMER_EXPECTED_RSP": 0x40b8, "TIMER_POST_RSP": 0x40b8, "TIMER_SAVED_RSP_FIELD": 0x40a0, "TIMER_SAVED_SS": 0x10, "IDLE_FRAME": 0x7000,
            "IDLE_INTERRUPTED_RSP": 0x70b0, "IDLE_SAVED_RSP_FIELD": 0x70a0, "IDLE_SAVED_SS": 0x10, "TEST_IDLE_SLOT": 1, "TEST_IDLE_GENERATION": 1, "BOOTSTRAP_SLOT": 0, "BOOTSTRAP_GENERATION": 1,
            "WORKER_SLOT": 2, "WORKER_GENERATION": 1, "IDLE_SLOT": 1, "IDLE_GENERATION": 1,
            "WORKER_SOFTWARE_FRAME": 0x8000, "WORKER_SOFTWARE_TASK_SLOT": 2, "WORKER_SOFTWARE_GENERATION": 1, "WORKER_SOFTWARE_RSP": 0x8100, "WORKER_SOFTWARE_SAVED_SS": 0x10,
            "WORKER_TIMER_FRAME": 0x9000, "WORKER_TIMER_TASK_SLOT": 2, "WORKER_TIMER_GENERATION": 1, "WORKER_TIMER_RSP": 0x9100, "WORKER_TIMER_SAVED_SS": 0x10,
            ...
            "INTERRUPT_CONTEXT_FAULT": 0, "SCHEDULER_ISR_ENTRIES": 0, "CURRENT_TASK_BEFORE_SLOT": 0, "CURRENT_TASK_BEFORE_GENERATION": 1, "CURRENT_TASK_AFTER_SLOT": 0, "CURRENT_TASK_AFTER_GENERATION": 1,
        }
    ...
    def test_timer_rejects_timeout_after_partial_markers_without_frame_error(self):
        output = "\n".join(TIMER_MARKERS[:TIMER_MARKERS.index("FINNOS:TEST:TIMER_INTERRUPTS:FREQUENCY_OK")])
        output = output.replace("FINNOS:TEST:TIMER_INTERRUPTS:PASS", "")
        errors = validate_timer(0, output + "\nFINNOS:TEST:TIMER_INTERRUPTS:REAL_TICKS_OK")
        self.assertTrue(any("partial timer markers" in error for error in errors))
        self.assertTrue(any("missing timer-frame error marker" in error for error in errors))
    ...
    def test_preemption_rejects_layout_and_pointer_contract_violations(self):
        numeric = {
            ...
            "WORKER_SOFTWARE_FRAME": 0x8000, "WORKER_SOFTWARE_TASK_SLOT": 2, "WORKER_SOFTWARE_GENERATION": 1, "WORKER_SOFTWARE_RSP": 0x8100, "WORKER_SOFTWARE_SAVED_SS": 0x10, "WORKER_TIMER_FRAME": 0x9000, "WORKER_TIMER_TASK_SLOT": 2, "WORKER_TIMER_GENERATION": 1, "WORKER_TIMER_RSP": 0x9100, "WORKER_TIMER_SAVED_SS": 0x10,
            "DEPTH_NESTED": 2, "DEPTH_INNER_DROPPED": 1, "DEPTH_OUTER_DROPPED": 0, "REQUEST_WHILE_NESTED": 1, "REQUEST_AFTER_INNER_DROP": 1, "REQUEST_AFTER_OUTER_DROP": 1, "REQUEST_TAKEN": 1, "REQUEST_AFTER_TAKE": 0, "TICK_DELTA": 1, "DELIVERY_DELTA": 1, "EOI_DELTA": 1, "SWITCHES_BEFORE": 4, "SWITCHES_AFTER": 4, "CR3_BEFORE": 0x1000, "CR3_AFTER": 0x1000, "CURRENT_TASK_BEFORE_SLOT": 0, "CURRENT_TASK_BEFORE_GENERATION": 1, "CURRENT_TASK_AFTER_SLOT": 0, "CURRENT_TASK_AFTER_GENERATION": 1, "IF_ENABLED": 1, "INTERRUPT_DEPTH": 0, "FAULTED": 0, "INTERRUPT_CONTEXT_FAULT": 0, "SCHEDULER_ISR_ENTRIES": 0,
        }
        lines = ["\n".join(PREEMPTION_CONTEXT_MARKERS)]
        lines.extend(f"FINNOS:PREEMPT:{key}={value:#x}" if key.endswith(("FRAME", "RIP", "RSP", "CR3", "LOOP_START", "LOOP_END")) else f"FINNOS:PREEMPT:{key}={value}" for key, value in numeric.items())
        patterns = [0x1111111111111111, 0x2222222222222222, 0x3333333333333333, 0x4444444444444444, 0x5555555555555555, 0x6666666666666666, 0x7777777777777777, 0x8888888888888888, 0x9999999999999999, 0xAAAAAAAAAAAAAAAA, 0xBBBBBBBBBBBBBBBB, 0xCCCCCCCCCCCCCCCC, 0xDDDDDDDDDDDDDDDD, 0xEEEEEEEEEEEEEEEE, 0xFFFFFFFFFFFFFFFF]
        for phase in ("SOFTWARE_SAVED", "SOFTWARE_POST", "TIMER_SAVED", "TIMER_POST"):
            lines.extend(f"FINNOS:PREEMPT:{phase}_R{index}=0x{value:x}" for index, value in enumerate(patterns))
        log = "\n".join(lines)
        self.assertTrue(validate_preemption_context(33, log.replace("SOFTWARE_RETURN_FRAME=0x1000", "SOFTWARE_RETURN_FRAME=0x1008")))
        self.assertTrue(validate_preemption_context(33, log.replace("SOFTWARE_LAYOUT=8", "SOFTWARE_LAYOUT=16")))
        inconsistent_rsp = log.replace("SOFTWARE_INTERRUPTED_RSP=0x10b8", "SOFTWARE_INTERRUPTED_RSP=0x10b0").replace("SOFTWARE_EXPECTED_RSP=0x10b8", "SOFTWARE_EXPECTED_RSP=0x10b0").replace("SOFTWARE_POST_RSP=0x10b8", "SOFTWARE_POST_RSP=0x10b0")
        self.assertTrue(validate_preemption_context(33, inconsistent_rsp))
