...
The x86-64 kernel now constructs and activates its own four-level identity-mapped page tables. Table pages come from the fixed-capacity physical allocator pool; kernel code is read-only executable, data and stack are writable NX, and the null page plus early-stack guard pages remain absent.
