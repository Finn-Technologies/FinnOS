...
The x86-64 kernel constructs its own four-level page tables, bounded physical allocator, guarded 1 MiB heap, xAPIC, and 100 Hz timer. It now also provides eight generation-tagged cooperative task slots, guarded 64 KiB task stacks, real SysV64 context switching, deferred stack reclamation, and a dedicated idle task. Its preemption-ready boundary saves a complete 176-byte ring-0 interrupt-return frame, validates dispatcher-selected returns and stack-derived attribution, and defers timer reschedule requests without switching in the ISR. Actual preemption, blocking, IOAPIC routing, device IRQs, user mode, drivers, and Peony remain unimplemented.
