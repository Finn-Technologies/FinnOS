...
Task stack slots begin at `0x0000280000000000`, are 128 KiB apart, and retain unmapped lower and upper guards plus fourteen unmapped padding pages. Validation checks every guard, padding page, leaf, recorded frame, and effective permission. Stack ownership metadata is non-copyable and tracks owned frames independently from mapped leaves, so a frame is recorded immediately after allocation. Creation and reclamation stage allocator changes, prevalidate every fallible leaf object, and reversibly update leaves before committing; a rollback failure retains both mapped and allocated-but-unmapped ownership and poisons further scheduling rather than freeing or reusing the slot. Returning task functions become `Exited`; another running task prepares the generation update, transactionally reclaims the stack, then commits the infallible policy reap. Intermediate page-table pages remain reserved while leaf and free-page baselines are restored.

The FIFO queue and task table allocate no heap memory. Policy transitions preflight fallible mutations; diagnostic counters permanently saturate at `u64::MAX`. Switch preparation applies the transition to a copied policy and commits it only after the selected runtime context and owned stack are valid. Runtime invariants couple policy state to entries, contexts, queue membership—including a suspended bootstrap—exclusive physical ownership, disjoint virtual ranges, and saved stack pointers. A switch saves `rsp`, `rbx`, `rbp`, and `r12`–`r15`; caller-saved registers follow SysV64. A synthetic frame resumes through `ret` into the trampoline. All tasks share CR3, and switching changes neither CR3 nor IF.
