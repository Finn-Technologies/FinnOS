...
`build/out/x86_64-qemu-page-allocator/` and verifies the allocator independently before paging activation. `./tools/finn test-page-tables` uses `build/out/x86_64-qemu-page-tables/` to verify the owned root, permissions, guards, scratch mapping, and controlled test sequence.
