...
The supported deployments are the x86-64 QEMU `q35`/OVMF path and the locally verified ARM64 R3 serial-entry slice on QEMU `virt`/AAVMF. UEFI firmware and the FinnOS boot manager are trusted to pass an aligned, mapped, immutable `BootInfo` pointer and to own and retain every physical range named by that structure through kernel entry. The x86 kernel rejects a null pointer, then dereferences a non-null pointer before structural validation; pointer provenance and mapping are therefore loader promises, not properties established by version 2. The R3 ARM64 kernel checks pointer presence/alignment but deliberately does not consume nested handoff data yet. Nested addresses remain integers while the consuming kernel validates the version, structure size, flags, lengths, descriptor metadata, and arithmetic bounds. Structural validation does not prove that a nested address is mapped, uniquely owned, immutable, or backed by the claimed resource.
